...
class _WebViewLessonScreenState extends ConsumerState<WebViewLessonScreen> {
  ...
  bool _tvvlChecked      = false;  // منع استدعاء increment أكثر من مرة للدرس ذاته
  ...
  Future<void> _loadLesson(Lesson lesson) async {
    ...
    // ── TVVL: تسجيل المشاهدة والتحقق من الحجب قبل فتح WebView ─────────────
    final tvvlResult = await _incrementAndCheckViews(lesson.id);
    if (!mounted) return;
    if (tvvlResult['is_blocked'] == true) {
      setState(() => _loading = false);
      _showTvvlBlockedDialog(tvvlResult);
      return;
    }
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // TVVL: تسجيل المشاهدة والتحقق من الحجب
  // ─────────────────────────────────────────────────────────────────────────

  /// يستدعي POST /app/v1/lesson-views/{id}/increment
  /// يُعيد map فيه is_blocked, wa_url, next_plan, ...
  /// عند فشل الطلب (network error) يُعيد is_blocked: false لتجنب حجب الدرس بسبب مشكلة شبكة
  Future<Map<String, dynamic>> _incrementAndCheckViews(int lessonId) async {
    if (_tvvlChecked) return {'is_blocked': false};
    _tvvlChecked = true;
    try {
      final resp = await DioClient.instance.dio.post(
        ApiConstants.lessonViewIncrementUrl(lessonId),
      );
      if (resp.statusCode == 200 && resp.data is Map) {
        final body = resp.data as Map<String, dynamic>;
        // الـ response مُغلَّف في { success: true, data: {...} }
        if (body['success'] == true && body['data'] is Map) {
          return Map<String, dynamic>.from(body['data'] as Map);
        }
      }
    } catch (_) {
      // فشل الاتصال — نسمح بالمشاهدة ولا نحجب
    }
    return {'is_blocked': false};
  }

  void _showTvvlBlockedDialog(Map<String, dynamic> data) {
    if (!mounted) return;
    final planName  = data['plan_name']  as String? ?? '';
    final viewsMax  = data['views_max'];
    final nextPlan  = data['next_plan'];
    final waUrl     = data['wa_url']     as String? ?? '';
    final nextLabel = nextPlan is Map ? (nextPlan['label'] as String? ?? '') : '';
    final maxInt    = viewsMax is int ? viewsMax : int.tryParse('$viewsMax') ?? 0;
          ...
          children: [
            ...
            if (nextPlan.isNotEmpty) ...[
              ...
              Container(
                ...
                child: Text('✨ الترقية إلى: $nextPlan',
